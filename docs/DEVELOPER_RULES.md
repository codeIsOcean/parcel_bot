...
## 1.1 Бот vs Mini App (контракт, см. `docs/BOT_VS_MINIAPP.md`)

Бот — тонкая обёртка: `/start` + онбординг (язык → телефон), кнопка «Открыть приложение»,
уведомления, оплата Stars, ответы поддержки, реестр групп. **Никаких** операционных
хендлеров, reply-меню и FSM-сценариев посылок/рейсов в боте. Новая функция = экран Mini App
+ эндпоинт API; в бот добавляется только уведомление с кнопкой на этот экран.

Диплинки — через `shared/deeplinks.py` и `webapp/src/utils/deeplinks.js` (один словарь).
